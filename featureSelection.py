...
def obtainFeatures(topics,docs):
	...
	for d in docs :
		...
		for t in topics :
			# calculo de fdj viejo.
			# if not(re.search(r'\b%s\b' % re.escape(t),document,re.IGNORECASE) is None) :
			# 	fdj[d-1] += 1

			count = sum(1 for _ in re.finditer(r'\b%s\b' % re.escape(t), document,re.IGNORECASE))
			fij[t].append(count)
